...
import { userForgotPass, userResetPass } from 'services/user';
...
export const ResetPassword: React.FC = () => {
	...
	const handleLogin = async () => {
		...
		if (!email || !reg.test(email)) {
			setError({ username: 'Sai định dạng email!', password: '' });
			return;
		}
		...
		try {
			...
			if (errResponse?.response?.data?.code === 'INVALID_EMAIL')
				setError({ username: 'Email không tồn tại, thử lại sau', password: '' });
			else setError({ username: 'Có lỗi xảy ra, thử lại sau', password: '' });
		}
	};
	...
	const handleResetpass = async () => {
		...
		const otpToken = otpRef.current?.value;
		...
		try {
			...
			history.replace('/auth/sing-in');
		} catch (err) {
			const errResponse = err as AxiosError<BaseResponseAction>;
			if (errResponse?.response?.data?.code === 'NOT_FOUND_OTP_TOKEN')
				setError({ password: 'Sai mã xác nhận, thử lại sau', username: '' });
			else setError({ password: 'Có lỗi xảy ra, thử lại sau', username: '' });
		}
	};
					...
					<TabPanels>
						...
						<TabPanel>
							<Box me="auto">
								...
								<Text mb="36px" ms="4px" color={textColorSecondary} fontWeight="400" fontSize="md">
									Nhập mật khẩu mới và mã xác thực từ email của bạn!
								</Text>
							</Box>
							<Flex
								zIndex="2"
